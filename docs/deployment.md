...
Важно: не запускайте `pnpm build` в активном каталоге, пока PM2 обслуживает тот же
релиз. Next.js очищает `.next` в начале build, а `postbuild` на короткое время
удаляет `public` и `.next/static` внутри standalone перед их копированием. В этот
промежуток живой процесс может отдать 404 для CSS/JS/изображений, а `/_next/image`
может записать `internal image response is empty` и вернуть 400. Это не ошибка
формата WebP или AVIF.
...
Текущая структура `/var/www/autozap` — in-place релиз. Для неё безопасный
минимальный порядок — короткое контролируемое окно обслуживания: сначала
подготовьте исходный код и зависимости, затем остановите PM2 **до** первого
изменения `.next`, и запускайте его только после полной успешной сборки. Не
выполняйте `pm2 restart` после build, если PM2 продолжал работать во время build.

Перед началом запишите текущий commit и убедитесь, что у вас есть проверенный
backup последнего рабочего релиза. Автоматический rollback не описан намеренно:
in-place build заменяет `.next`, и восстановление без заранее сохранённого
артефакта небезопасно.
...
# Не запускайте db:migrate в обычном деплое. Миграции требуют отдельного
# согласования, backup и плана отката.
pnpm lint
pnpm typecheck

# С этого момента сайт намеренно недоступен, зато не отдаёт смешанные релизы.
pm2 stop autozap
pnpm build
DEPLOY_USER=autozap
sudo chown -R "$DEPLOY_USER":"$DEPLOY_USER" /var/www/autozap
...
curl -fsS 'http://127.0.0.1:3000/_next/image?url=%2Fassets%2Fstore%2Ffacade.webp&w=1920&q=75' > /dev/null
```

Если любая команда после `pm2 stop` завершается с ошибкой, не запускайте PM2 с
частичной `.next`. Восстановите заранее сохранённый рабочий релиз по локальному
runbook, затем выполните его проверку и только после этого запустите `autozap`.

Zero-downtime деплой требует отдельной миграции на схему `releases/<id>` +
атомарный symlink `current` и смены `cwd` PM2. Не добавляйте её частично: сначала
нужно проверить фактические PM2/nginx-конфигурации на сервере и способ хранения
`public/uploads`.
