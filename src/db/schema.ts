...
export const reviewRecalculations = pgTable(
  "review_recalculations",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    workspaceId: uuid("workspace_id")
      .notNull()
    ...
    rulesVersion: varchar("rules_version", { length: 160 }).notNull(),
    commitHash: varchar("commit_hash", { length: 64 }),
    inputFingerprint: varchar("input_fingerprint", { length: 128 }).notNull(),
    previewToken: varchar("preview_token", { length: 128 }).notNull(),
  ...
  (table) => ({
    workspaceStatusIdx: index("review_recalculations_workspace_status_idx").on(
      table.workspaceId,
      table.status,
      table.createdAt
    ),
    oneRunningWorkspaceIdx: uniqueIndex("review_recalculations_one_running_workspace_idx")
      .on(table.workspaceId)
      .where(sql`${table.status} = 'running'`)
  })
);

export const reviewRecalculationItems = pgTable(
  "review_recalculation_items",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    recalculationId: uuid("recalculation_id")
      .notNull()
      .references(() => reviewRecalculations.id, { onDelete: "cascade" }),
    reviewQueueId: uuid("review_queue_id").references(() => reviewQueue.id, {
      onDelete: "set null"
    }),
    productId: uuid("product_id")
      .notNull()
      .references(() => products.id, { onDelete: "cascade" }),
    oldSuggestedCategoryId: uuid("old_suggested_category_id").references(() => categories.id, {
      onDelete: "set null"
    }),
    oldSuggestedSubcategoryId: uuid("old_suggested_subcategory_id").references(() => subcategories.id, {
      onDelete: "set null"
    }),
    newSuggestedCategoryId: uuid("new_suggested_category_id").references(() => categories.id, {
      onDelete: "set null"
    }),
    newSuggestedSubcategoryId: uuid("new_suggested_subcategory_id").references(() => subcategories.id, {
      onDelete: "set null"
    }),
    oldReason: text("old_reason"),
    newReason: text("new_reason").notNull(),
    decisionStatus: varchar("decision_status", { length: 40 }).notNull(),
    decisionSource: varchar("decision_source", { length: 80 }).notNull(),
    ...
    familyLabel: varchar("family_label", { length: 180 }),
    matchedSignals: jsonb("matched_signals").notNull().default(sql`'[]'::jsonb`),
    negativeSignals: jsonb("negative_signals").notNull().default(sql`'[]'::jsonb`),
    candidates: jsonb("candidates").notNull().default(sql`'[]'::jsonb`),
    isOtherProducts: boolean("is_other_products").notNull().default(false),
    isExactAssignment: boolean("is_exact_assignment").notNull().default(false),
    originalProductStatus: varchar("original_product_status", { length: 64 }),
    metadata: jsonb("metadata").notNull().default(sql`'{}'::jsonb`),
    ...timestamps
  },
  (table) => ({
    recalculationReviewUnique: unique("review_recalculation_items_recalc_review_unique").on(
      table.recalculationId,
      table.reviewQueueId
    ),
    recalculationStatusIdx: index("review_recalculation_items_recalc_status_idx").on(
      table.recalculationId,
      table.decisionStatus
    ),
    recalculationProductIdx: index("review_recalculation_items_product_idx").on(table.productId)
  })
);
